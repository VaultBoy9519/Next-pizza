...
export const Stories: React.FC<Props> = ({ className }) => {
	...
	return (
		<>
			<Container className={cn('flex scrollbar items-center w-full overflow-auto justify-between gap-2 my-10', className)}>
				...
				{open && (
					<div className='absolute left-0 top-0 w-full h-full bg-black/80 flex items-center justify-center z-30'>
						<div className='relative'>
							...
							<ReactStories onAllStoriesEnd={() => setOpen(false)} stories={selectedStory?.items.map(item => ({ url: item.sourceUrl })) || []} defaultInterval={3000} width={520} height={800} />
						</div>
					</div>
				)}
			</Container>
